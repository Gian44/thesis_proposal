...
class TimetableSolverSA:
    ...
    def try_swap(self, unassigned_course, blocking_assignment, day, period, room, solution, visited_courses=None):
        """
        Attempt to swap the blocking course with the unassigned course recursively.
        """
        if visited_courses is None:
            visited_courses = set()  # Initialize a set to track visited courses

        # Mark the blocking course as visited to avoid infinite loops
        visited_courses.add(blocking_assignment.course.id)

        # Remove the blocking assignment temporarily
        solution.remove_assignment(blocking_assignment)

        # Check if the unassigned course can fit in the blocking assignment's slot
        if self.is_feasible_assignment(unassigned_course, day, period, room, solution) and not solution.is_course_scheduled_at(unassigned_course.id, day, period):
            ...
            for new_day in range(self.days):
                for new_period in range(self.periods_per_day):
                    for new_room in self.rooms:
                        # Check if the blocking course can be assigned without duplicating its timeslot
                        if (
                            self.is_feasible_assignment(blocking_assignment.course, new_day, new_period, new_room, solution)
                            and not solution.is_course_scheduled_at(blocking_assignment.course.id, new_day, new_period)
                        ):
            ...
            # If the blocking course couldn't be reassigned directly, try recursively swapping
            if not reassigned:
                for new_day in range(self.days):
                    for new_period in range(self.periods_per_day):
                        for new_room in self.rooms:
                            assignments_to_consider = solution.get_assignments_for_period(new_day, new_period)
                            for new_blocking_assignment in assignments_to_consider:
                                # Avoid swapping with already visited courses
                                if new_blocking_assignment.course.id in visited_courses:
                                    continue
                                # Recursively attempt to swap
                                if self.try_swap(blocking_assignment.course, new_blocking_assignment, new_day, new_period, new_room, solution, visited_courses):
                                    reassigned = True
                                    break
                            if reassigned:
                                break
            ...
            # If the blocking course cannot be reassigned even after recursive swaps, undo the changes
            if not reassigned:
        ...
        # If the unassigned course cannot be assigned, re-add the blocking assignment
        solution.assign_course(blocking_assignment.course, blocking_assignment.room_id, blocking_assignment.day, blocking_assignment.period)
        return False
